...
import { ACCESS_TOKEN, ACCESS_NUMBER } from '@/store/mutation-types'
...
router.beforeEach((to, from, next) => {
  ...
  if (Vue.ls.get(ACCESS_TOKEN)) {
    ...
    if (to.path === '/user/login') {
      next({ path: '/dashboard/home/homePage' })
      NProgress.done()
    } else if (store.getters.roles.length === 0) {
      store.dispatch('GetInfo').then(res => {
        if (res.message === 'SUCCESS') {
          const access_number = Vue.ls.get(ACCESS_NUMBER)
          if (access_number === 1) {
            setTimeout(() => {
              notification.success({
                message: `${res.data.nickName},您好`,
        ...
        store.dispatch('GenerateRoutes', { roles }).then(() => {
          ...
          if (to.path === redirect) {
            ...
            if (newPath[3].indexOf('_Detail') !== -1 || newPath[3].indexOf('_Edit') !== -1) {
              ...
              newRedirect = newPath[0] + '/' + newPath[1] + '/' + newPath[2] + '/' + newName
            } else {
