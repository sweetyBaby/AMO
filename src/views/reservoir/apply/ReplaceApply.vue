        ...
        >
          <a-form layout="inline">
            <div class="searchWrap">
              ...
              <div class="searchList">
                <label for>是否有SN</label>
                <a-select
                  v-decorator="['hasSerialNumber', { rules: [{ required: true, message: '请选择是否有SN！', whitespace: true }]}]"
                  placeholder="请选择是否有SN"
                  style="width: 100%"
                  v-model="queryParam.hasSerialNumber"
                >
                  <a-select-option
                    v-for=" (item,index) in SNcheck"
                    :value="item.code "
                    :key="index"
              ...
              <div class="searchList">
                ...
                <a-select
                  v-decorator="['applyResult', { rules: [{ required: true, message: '请选择申请状态！', whitespace: true }]}]"
                  placeholder="请选择申请状态"
                  style="width: 100%"
...
export default {
  ...
  created () {
    // console.info('this.$route.meta.title=====', this.$route.meta.title);
  ...
  methods: {
    ...
    getFirstLoad () {
      ...
      if (this.hasPermission('pass') || this.hasPermission('unpass') && !isAction) {
        this.columns.push({
          width: 100,
          align: 'center',
          dataIndex: 'actions',
    ...
    rowShow () {
      return this.hasPermission('allPass') === true
        ? {
    ...
    handleAdd () {
      ...
      history.pushState(null, null, '/reservoir/apply/ReplaceApply#detail')
  ...
  watch: {
    $route (to, from) {
      // console.info('t0------', to)
      if (to.path === '/reservoir/apply/ReplaceApproval' || to.path === '/reservoir/apply/ReplaceApply') {
        this.getFirstLoad()
      }
    }
  },
