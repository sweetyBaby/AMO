<template>
  <div>
    <div class="pageTitle">
      <a-row :gutter="48">
        <a-col :md="14">
          <h3 v-if="isAllData">{{ titleInfo.auditTitle }}</h3>
        </a-col>
        <a-col :md="10">
    ...
    <!-- <div class="pageTitle pageTitleMerge" v-else>
      <a-row :gutter="48">
        <a-col :md="14">
...
export default {
  ...
  created() {
    ...
    const titleInfo = JSON.parse(localStorage.getItem('estateDetail'))// 区分是编辑  新增 及标题
    const InvoiceId = getStore('EstateId') // 详情信息
    ...
    if (titleInfo.type === 'add') {
      this.titleData = {
        type: 'add',
        addType: titleInfo.addtype, // 区分是哪种新增方式   除了图片及扫描
      ...
      if (titleInfo.isImgAdd || titleInfo.auditTitle === '图片发票上传') {
        this.isEdit = true
        this.isAllData = true
      }
  ...
  methods: {
    ...
    handleUpEstate(payload) {
      ...
      this.titleData.addType = addtype
      this.titleInfo.addType = addtype
      this.titleInfo.auditTitle = auditTitle
      this.isAllData = true
      this.isEdit = true
    }
  }
