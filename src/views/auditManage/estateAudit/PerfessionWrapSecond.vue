<template>
  <a-card :bordered="false" v-if="pageIndex">
    ...
    <div class="scanEmptyMerge" v-else>
      <EstateEmpty @click="handleUpEstate" />
    </div>
  </a-card>
  ...
  <a-card :bordered="false" v-else-if="pageDetails">
    <AddNewEstateSecond @details="scanBack" :detailsDataProp="detailsData" @add="handleUpEstate"></AddNewEstateSecond>
  </a-card>
</template>
...
import AddNewEstateSecond from './PerfessionSecond_Edit'
...
export default {
  ...
  methods: {
    ...
    getFirstLoad() {
      ...
      this.queryParam.distributionInvoiceId = ''
    ...
    setRouterState() {
      ...
      let parmPath = '/auditManage/estateAudit/PerfessionWrapSecond#eclosure'
      if (this.type && this.type === 'supple') {
    ...
    handleAdd() {
      ...
      this.pageManual = false
      this.pageDetails = true
      localStorage.setItem('estateDetail', JSON.stringify(this.detailsData))
    },
    scanBack() {
      // 扫码返回
      this.isEmpty = false
