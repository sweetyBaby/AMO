      ...
      <a-row :gutter="48">
        ...
        <a-col :md="8" :sm="24">
          <div class="icons-list pageTitle-Btn">
            ...
            <span class="titleBtn" @click="downloadTemplate" v-if="hasPermission('down')">
              <a-tooltip placement="top">
                <template slot="title">
                  <span>下载模板</span>
                </template>
            ...
            <span class="titleBtn" @click="scanUpload" v-if="hasPermission('scan')">
              <a-tooltip placement="top">
                ...
                <i class="iconfont icon-scanIcon"></i>
              </a-tooltip>
            </span>
            ...
            <span class="titleBtn" v-if="hasPermission('excel')">
              <a-upload
                action="/amo/invoice/importInvoiceExcel"
                @change="uploadChangeExcel"
            ...
            <div class="searchList">
              ...
              <div class="basicInfo">
                <a-form-item style="width: 100%" has-feedback>
                  ...
                  <!--                  >-->
                  <!--                    <a-select-option-->
                  <!--                      v-for="(item,key) in buyerData"-->
                  <!--                      :value="item.code"-->
                  <!--                      :type="item.type"-->
                  <!--                      :key="key"-->
                  <!--                    >{{ item.name }}</a-select-option>-->
                  <!--                  </a-select>-->
                  <a-input v-model="queryParam.cusName" placeholder="请输入购货方"></a-input>
                </a-form-item>
              </div>
            </div>
            <div class="searchList">
              ...
              <a-popover placement="bottomRight" v-model="record.isVoid">
                <template slot="content">
                  <p @click="handleDetails(text,record)" v-show="record.status !== '2'">
                    ...
                    {{ (record.productFlag === '0') ? '发票明细' : '查看详情' }}
                  </p>
...
import { DataFilter, exportTable, BtnShow, exportTabList, setStore } from '@/utils/storage'
...
import AuditEdit from './modules/auditEdit'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        return invoiceList(Object.assign(this.queryParam, parameter)).then(
          res => {
  ...
  methods: {
    ...
    getFirstLoad () {
      ...
      const _that = this
      if (
        this.hasPermission('down') ||
          this.hasPermission(
            'scan' || this.hasPermission('imgAdd') || this.hasPermission('add')
          )
      ) {
    ...
    handleDetails (text, record) {
      // 添加明细
      this.detailsData.id = record.id
      this.detailsData.status = record.status
      this.detailsData.verifyStatus = record.verifyStatus
      this.detailsData.verifyErrCode = record.verifyErrCode
      this.detailsData.verifyErrMsg = record.verifyErrMsg
      const params = {
        addType: null
      }
      setStore('AUDITEDIT', {})
      setStore('EstateInfo', JSON.stringify(params))
      setStore('EstateId', JSON.stringify(this.detailsData))
      this.$router.push({
        name: 'AuditManage_Detail'
      })
      // window.localStorage.setItem('EstateInfo', JSON.stringify(params))
    },
    scanBack () {
      // 扫码返回
      this.pageIndex = true
