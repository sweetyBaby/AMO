<template>
  <div>
      ...
      <a-row :gutter="48">
        ...
        <a-col :md="10">
          <div class="icons-list pageTitle-Btn">
            ...
            <span class="titleBtn" @click="manualSearch" v-if="titleData.status !== '2' && titleData.verifyStatus !== '05' && hasPermission('add')">
              <a-tooltip placement="top">
                <template slot="title">
                  <span>手动选择</span>
                </template>
            ...
            <span class="titleBtn" @click="accordCode" v-if="titleData.status !== '2' && titleData.verifyStatus !== '05' && hasPermission('add')">
              <a-tooltip placement="top">
                <template slot="title">
                  <span>根据单号</span>
                </template>
            ...
            <span class="titleBtn" v-if="titleData.status !== '2' && titleData.verifyStatus !== '05' && hasPermission('add')">
              <a-upload
    ...
    <div style="position:relative" v-if="turnOn">
      <div class="scanListAudit">
        <div class="invoiceDelete" @click="editTitleData" v-if="titleData.status!=='2' && titleData.verifyStatus !== '03' && titleData.verifyStatus !== '05'">
          <a-tooltip placement="top">
            <template slot="title">
              <span>编辑</span>
            </template>
          ...
          <a-row>
            ...
            <a-col :span="8">
              <a-row class="tablecell">
                ...
                <a-col :span="18" class="rightcell" :title="titleData.salesYear + '-' + titleData.salesMonth">{{ titleData.salesYear + '-' + titleData.salesMonth }}</a-col>
              </a-row>
            </a-col>
            <a-col :span="8">
              <a-row class="tablecell">
            ...
            <a-col :span="8">
              <a-row class="tablecell">
                ...
                <a-col :span="18" class="rightcell" :title="titleData.statusText">{{ titleData.statusText }}</a-col>
              </a-row>
            </a-col>
            <a-col :span="16">
              <a-row class="tablecell">
                <a-col :span="3" class="leftcell" title="备注" :class="titleData.verifyCodeShow ? 'leftcell borderRadiusLeft' : 'leftcell'">备注</a-col>
                <a-col :span="21" class="rightcell" :title="titleData.inRemark" :style="titleData.verifyCodeShow ? remarkSty : 'border-radius:0 0 8px 0;'">{{ titleData.inRemark }}</a-col>
              </a-row>
            </a-col>
          </a-row>
        </div>
      ...
      <a-tooltip>
        ...
        <a href="javascript:;" class="iconCheck">
          <i class="iconfont check" :class="checkStyle(titleData)"></i>
        </a>
      </a-tooltip>
    </div>
    <div class="addDeatils" v-if="titleData.status!=='2' && titleData.verifyStatus === '03' && this.hasPermission('add')" v-show="isEmpty">
      ...
      <div>
        ...
        <a-table
          ...
          <span slot="qty" slot-scope="text, record, index">
            <template>
              <div v-if="titleData.status !== '2' && titleData.verifyStatus !== '05' && hasPermission('add')" class="detailsProduct">
                <span v-show="tableNegative">-</span>
                <a-input style="margin: -5px 0" v-model="record.newQty" @change="changeQty(record)" @blur="blurQty(record)" v-show="titleData.verifyStatus !== '05'"></a-input>
              </div>
              <div v-else>
                <span v-show="tableNegative">-</span>
                {{ record.qty }}
          ...
          <span slot="nonePrice" slot-scope="text, record">
            <template>
              <div v-if="titleData.status!=='2' && titleData.verifyStatus !== '05' && hasPermission('add')">
      ...
      <div class="accompanyUpload">
        <div class="clearfix">
          <a-upload
            ...
            :beforeUpload="beforeUploadSui"
            :data="accompanyList.param">
            <div v-if="accompanyList.fileList.length < 9 && titleData.verifyStatus !== '05'">
...
export default {
  ...
  mounted() {
    this.titleData = JSON.parse(JSON.stringify(this.detailsDataProp))
    this.titleData.salesMonth = this.titleData.salesMonth ? this.titleData.salesMonth : ''
    this.titleData.salesYear = this.titleData.salesYear ? this.titleData.salesYear : ''
  },
  methods: {
    moment,
    hasPermission(val) {
    ...
    getFirstLogin() {
      ...
      if (this.hasPermission('add')) {
        const isExist = this.columns.findIndex(item => item.title === '操作')
        if (isExist === -1) {
          this.columns.push({
            title: '操作',
            align: 'center',
    ...
    getDetail(res) {
      ...
      this.titleData.statusText = this.titleData.inStatus === '1' ? '蓝票' : '红票'
      ...
      this.errMsg = this.titleData.verifyErrMsg
      this.$forceUpdate()
      this.$nextTick(() => {
        this.titleData.InvoiceId = this.titleData.id
        // console.info('this.titleData---', this.titleData)
        setStore('EstateId', this.titleData)
      })
    },
    getInType(intyppe) {
      switch (intyppe) {
        case '01':
          this.titleData.inTypeText = '增值税专用发票'
          this.titleData.verifyCodeShow = false
          break
        case '04':
          this.titleData.inTypeText = '增值税普通发票'
          this.titleData.verifyCodeShow = true
          break
        case '10':
          this.titleData.inTypeText = '增值税普通发票（电子）'
          this.titleData.verifyCodeShow = true
          break
        case '97':
          this.titleData.inTypeText = '机打发票'
          this.titleData.verifyCodeShow = true
          break
        case '98':
          this.titleData.inTypeText = '冠名票'
          this.titleData.verifyCodeShow = true
          break
        case '99':
          this.titleData.inTypeText = '手术植入单'
          this.titleData.verifyCodeShow = true
          break
      }
    },
