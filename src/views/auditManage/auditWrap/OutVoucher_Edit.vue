            ...
            <span slot="qty" slot-scope="text, record, index">
              <template>
                <div class="detailsProduct">
                  ...
                  <a-input
                    style="margin: -5px 0"
                    v-model="record.newQty"
                    @change="changeQty(record)"
                    @blur="blurQty(record)"
                    :disabled="isClosed"
                  ></a-input>
                </div>
            ...
            <span slot="nonePrice" slot-scope="text, record">
              <template>
                <div>
                  <a-input
                    style="margin: -5px 0"
                    v-model="record.nonePrice"
                    @change="changeNonePrice(record)"
                    @blur="blurNonePrice(record)"
                    :disabled="isClosed"
                  ></a-input>
                </div>
              </template>
...
import OutVocherEdit from './modules/OutVocherEdit'
import OutVocherDetail from './modules/OutVoucherDetail'
...
export default {
  ...
  methods: {
    moment,
    getFirst () {
      const newData = getStore('outVDetail')
    ...
    handleEstSub (val) {
      const newData = JSON.parse(window.localStorage.getItem('outVDetail'))
      this.isEdit = false
      this.titleData = {
        ...newData,
        ...val
    ...
    handleReset () {
      ...
      this.titleData = {}
      this.isEmpty = true
      window.localStorage.setItem('outVDetail', {})
    },
    handleSubmit () {
      this.handleReset()
    },
  ...
  watch: {
    $route (to, from) {
      if (to.name === 'OutVoucher_Edit' || to.name === 'OutVoucherSupple_Edit' || to.name === 'OutVoucherAppeal_Edit') {
