    ...
    <a-form layout="inline" :form="form">
      <div class="basicBorder">
        <a-row :gutter="24">
          ...
          <a-col span="8">
            <label for class="basicTitle">总计</label>
            <div class="basicInfo">
              <a-input
                disabled
      ...
      >
        <div class="newHosTitle">
          ...
          <label for class="basicTitle">小计</label>

          <a-input
            disabled
            :value="totalNUm(index,modelData)"
            :style="{ maxWidth:'100px',width:'100%' ,marginTop:'8px'}"
            :maxLength="100"
          ></a-input>
        ...
        <a-row :gutter="24" v-for="(product,index) in model.proList" :key="index">
          <a-col span="16">
            <label for class="basicTitle">
              <span>*</span>度数
            </label>
...
export default {
  ...
  methods: {
    totalNUm(index, modal) {
      let num = 0;
      console.log(modal, "====");
      modal[index].proList.forEach(val => {
        num += parseFloat(val.qty);
      });
      return num;
    },
    handleChange(e) {
      ...
      getCrgdPro().then(res => {
        ...
        if (res.message === "SUCCESS") {
          ...
          res.data.map(item => {
            if (
              modelData.findIndex(
                list => list.productModel === item.productModel
              ) === -1
            ) {
              modelData.push(item);
            }
    ...
    handelDel(product, productModel) {
      ...
      this.modelData.map(item => {
        if (item.productModel === productModel) {
          item.proList = item.proList.filter(
            obj => obj.degree !== product.degree
          );
        }
      });
    },
    ...
    handleModleSub(formdata) {
      ...
      oldData.map((old, index) => {
        if (
          old.productModel === formdata.orderDetails[0].productModel &&
          old.degree === formdata.orderDetails[0].degree
        ) {
          if (
            formdata.orderDetails[0].rowkey &&
            old.rowkey === formdata.orderDetails[0].rowkey
          ) {
      ...
      if (modify < 1) {
        if (!formdata.orderDetails[0].rowkey) {
          const newInfo = {
            rowkey:
              formdata.orderDetails[0].productModel +
              "_" +
    ...
    handleSelect(val) {
      ...
      this.proNoList.map(item => {
        if (item.productModel === val) {
          const degreeId = this.degreeList.findIndex(
            obj => obj.degree === item.degree
          );
          if (degreeId < 0) {
      ...
      this.degreeList.sort(
        (x, y) => parseFloat(x.degree) - parseFloat(y.degree)
      );
    },
    handleMClose() {
    ...
    handleMolSub(checkList) {
      ...
      checkData.map(item => {
        ...
        if (selectList.type === "model") {
          checkIndex = this.modelData.findIndex(
            obj => obj.productModel === item
          );
          if (checkIndex === -1) {
            newList.push({
              productModel: item,
