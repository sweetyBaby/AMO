<template>
  ...
  <a-card v-else :bordered="false">
    <add-dealers-host :tabletData="tabletInfo" @handleNewPro="handleChange()" />
  </a-card>
</template>
...
import { retriveAccreditHost, deleDealersHost, retriveDealersData } from '@/api/basicInfo/hosData'
import AddDealersHost from './modules/AddDealersHost'
...
export default {
  ...
  methods: {
    ...
    handleAdd(val) {
      ...
      history.pushState(null, null, '/more/basicInfo/DealersAccreditHost#detail')
    ...
    deleHostData(record) {
      this.$confirm({
        title: '删除确认',
        content: '是否确定删除【' + record.distName + '】数据？',
        okText: '确定',
        cancelText: '取消',
        onOk: () => {
          const params = {
          ...
          deleDealersHost(params).then(res => {
