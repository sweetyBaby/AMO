...
import { hosQuoList, hosQuoDelete } from '@/api/basicInfo/hosData'
...
export default {
  ...
  data () {
    return {
      ...
      isEdit: false,
      loadData: parameter => {
        return hosQuoList(Object.assign(this.queryParam, parameter)).then(
          res => {
  ...
  methods: {
    ...
    getFirstLoad () {
      ...
      if (this.hasPermission('edit') || this.hasPermission('delete') || this.hasPermission('detail')) {
        this.columns.push({
          width: 100,
          align: 'center',
          title: '操作',
    ...
    handleResult (record) {
      const params = {
        backUrl: 'HospitalSales',
        type: 'hosSales',
        data: record
      }
      setStore('HOSSALES', params)
      this.$router.push({
        path: 'HospitalSales_Detail'
      })
    },
    handleCancle () {
