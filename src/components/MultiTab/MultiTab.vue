...
export default {
  ...
  watch: {
    '$route': function (newVal) {
      // console.info('newVal++++++',this.isTabCheck)
      // console.info('this.fullPathList=====',this.fullPathList)
    ...
    activeKey: function (newPathKey) {
      // console.info('newPathKey====',this.$router)

      this.$router.options.routes.map((item) => {
        if (item.path === '/') {
          item.children.map((leveFirst) => {
            leveFirst.children.map(second => {
              second.children.map(third => {
                if (third.path === newPathKey && this.isTabCheck) {
                  third.meta.keepAlive = false
                } else if (third.path === newPathKey && !this.isTabCheck) {
                  third.meta.keepAlive = true
                }
              })
            })
          })
